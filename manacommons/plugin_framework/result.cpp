...
{
	_data = std::make_shared<io::OutputTreeNode>(plugin_name, io::OutputTreeNode::LIST);
	_data->append(std::make_shared<io::OutputTreeNode>("level", NO_OPINION));
	_data->append(std::make_shared<io::OutputTreeNode>("plugin_output", io::OutputTreeNode::LIST));
}
...
void Result::set_level(LEVEL level)
{
	...
	if (!opt_level) // Should never happen.
	{
		PRINT_WARNING << "[Result] A result object has no level node. This should be investigated."
			<< DEBUG_INFO << std::endl;
		_data->append(std::make_shared<io::OutputTreeNode>("level", level));
	}
	else {
...
void Result::set_summary(const std::string& s)
{
	...
	if (!opt_summary) {
		_data->append(std::make_shared<io::OutputTreeNode>("summary", s));
	}
	else {
...
io::pNode Result::get_information() const
{
	...
	if (!output)
	{
		PRINT_WARNING << "[Result] A result object's output data wasn't initialized!"
			<< DEBUG_INFO << std::endl;
		output = std::make_shared<io::OutputTreeNode>("plugin_output", io::OutputTreeNode::LIST);
		_data->append(output);
	}
	return output;
}
